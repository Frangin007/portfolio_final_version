...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: '#0f1117', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, opacity: 0.03, zIndex: 1, backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.75' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E")`, backgroundSize: '200px' }} />
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, backgroundImage: 'radial-gradient(rgba(255,255,255,0.035) 1px, transparent 1px)', backgroundSize: '36px 36px' }} />
      ...
      <div style={{ width: '100%', padding: 'clamp(120px,14vw,160px) clamp(20px,6vw,80px) clamp(80px,8vw,100px)', position: 'relative', zIndex: 2 }}>
        ...
        {/* headline */}
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(3rem,8vw,8.5rem)', fontWeight: 800, lineHeight: 0.97, letterSpacing: '-0.04em', color: '#f0f6ff', marginBottom: '24px', wordBreak: 'break-word' as const }}>
          Je construis<br />
        ...
        <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', alignItems: 'center', marginBottom: 'clamp(56px,8vw,90px)' }}>
          <button onClick={() => goto('contact')} style={{ padding: 'clamp(13px,2vw,16px) clamp(28px,3.5vw,40px)', background: '#38bdf8', color: '#040710', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'clamp(0.95rem,1.5vw,1.05rem)', border: 'none', borderRadius: '12px', cursor: 'pointer', transition: 'all 0.3s', display: 'flex', alignItems: 'center', gap: '9px' }}
