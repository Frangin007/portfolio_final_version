...
const Navigation: React.FC = () => {
  ...
  return (
    <nav style={{
      ...
      background: scrolled || open ? 'rgba(26,29,46,0.96)' : 'transparent',
      ...
      <div style={{ width: '100%', padding: '0 clamp(20px, 6vw, 80px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '72px' }}>
          ...
          {/* Desktop links — masqués sous 768px via CSS */}
          ...
          {/* Burger — visible uniquement sous 768px via CSS */}
          <button className="nav-burger" onClick={() => setOpen(!open)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text)', padding: '8px', borderRadius: '10px', transition: 'background 0.2s', alignItems: 'center', justifyContent: 'center' }}
      ...
      {/* Mobile menu — animé avec max-height */}
      <div style={{
        overflow: 'hidden',
        ...
        <div style={{ padding: '12px clamp(20px, 6vw, 80px) 24px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {navItems.map(item => (
            <button key={item.id} onClick={() => goto(item.id)}
              style={{ display: 'block', width: '100%', textAlign: 'left', padding: '14px 16px', borderRadius: '12px', fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: active === item.id ? 700 : 500, background: active === item.id ? 'rgba(56,189,248,0.1)' : 'transparent', color: active === item.id ? '#38bdf8' : 'var(--text2)', border: 'none', cursor: 'pointer', transition: 'all 0.2s ease' }}>
              {item.label}
            </button>
          ))}
          <button onClick={() => goto('contact')}
            style={{ marginTop: '8px', padding: '15px 16px', borderRadius: '12px', fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: 700, background: '#38bdf8', color: '#040710', border: 'none', cursor: 'pointer', width: '100%' }}>
            Hire me →
          </button>
        </div>
      </div>
